...
describe('POST /api/auth/signup', () => {
  ...
  it('should register a patient successfully', async () => {
    ...
    // No token in body — tokens are in cookies now
  ...
  it('should return 422 if email is invalid', async () => {
    const res = await request(app).post('/api/auth/signup').send({ ...validPatient, email: 'notanemail' });
    expect(res.status).toBe(422);
  });

  it('should return 422 if password is too short', async () => {
    const res = await request(app).post('/api/auth/signup').send({ ...validPatient, password: '123' });
    expect(res.status).toBe(422);
  });
  ...
  it('should return 422 if role is invalid', async () => {
    const res = await request(app).post('/api/auth/signup').send({ ...validPatient, role: 'admin' });
    expect(res.status).toBe(422);
  });
